...
describe('Jina SDK tweet example tests', () => {
    ...
    let feeling: JinaChatOutput | null = null;

    it('JinaChat: get the general feeling according to the tweets', async () => {
        const prompt = [
            'According to those tweets, is the general feeling positive or negative?',
            'Reply by [POSITIVE] or [NEGATIVE]',
            ...positiveMovieTweets.map(t => 'TWEET:\n' + t),
        ].join('\n');
        feeling = await jinaai.generate(prompt);
        console.log('GENERAL FEELING: ', feeling.output);
        expect(feeling.output).toBeTruthy();
        expect(feeling.output.length > 0).toBeTruthy();
        expect(feeling.chatId).toBeTruthy();
        expect(feeling.output.includes('POSITIVE')).toBeTruthy();
    });
    ...
    it('JinaChat: get the general feeling according to the tweets', async () => {
        const prompt = [
            'According to those tweets, is the general feeling positive or negative?',
            'Reply by [POSITIVE] or [NEGATIVE]',
            ...negativeMovieTweets.map(t => 'TWEET:\n' + t),
        ].join('\n');
        feeling = await jinaai.generate(prompt);
        console.log('GENERAL FEELING: ', feeling.output);
        expect(feeling.output).toBeTruthy();
        expect(feeling.output.length > 0).toBeTruthy();
        expect(feeling.chatId).toBeTruthy();
        expect(feeling.output.includes('NEGATIVE')).toBeTruthy();
    });
